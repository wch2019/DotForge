...
function getDb() {
  ...
  let dbPath;
  if (isDir) {
    const dataDir = require$$1$1.join(CONFIG_PATH2, "data");
    fs.ensureDirSync(dataDir);
    dbPath = require$$1$1.join(dataDir, "data.db");
  } else {
    fs.ensureDirSync(require$$1$1.dirname(CONFIG_PATH2));
    dbPath = CONFIG_PATH2;
  }
...
function registerSSHHandlers() {
  ...
  ipcMain.handle("ssh:execute", async (_, connectionId, command) => {
    try {
  ...
  ipcMain.handle("ssh:getSystemInfo", async (_, connectionId) => {
    try {
  ...
  ipcMain.handle("ssh:isConnected", async (_, connectionId) => {
