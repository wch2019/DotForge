...
if (!fs$q[gracefulQueue]) {
  ...
  if (/\bgfs4\b/i.test(process.env.NODE_DEBUG || "")) {
    process.on("exit", function() {
      ...
      require$$5$1.equal(fs$q[gracefulQueue].length, 0);
    });
  }
}
...
function pathExists$6(path2) {
  return fs$o.access(path2).then(() => true).catch(() => false);
}
var pathExists_1 = {
...
async function utimesMillis$1(path2, atime, mtime) {
  const fd = await fs$n.open(path2, "r+");
  let closeErr = null;
  try {
    await fs$n.futimes(fd, atime, mtime);
  } finally {
    try {
      await fs$n.close(fd);
    } catch (e) {
...
function getStats$1(src, dest, opts) {
  const statFunc = opts.dereference ? (file2) => fs$m.stat(file2, { bigint: true }) : (file2) => fs$m.lstat(file2, { bigint: true });
  return Promise.all([
    statFunc(src),
...
function getStatsSync(src, dest, opts) {
  let destStat;
  const statFunc = opts.dereference ? (file2) => fs$m.statSync(file2, { bigint: true }) : (file2) => fs$m.lstatSync(file2, { bigint: true });
...
async function checkParentPaths(src, srcStat, dest, funcName) {
  ...
  let destStat;
  try {
    destStat = await fs$m.stat(destParent, { bigint: true });
  } catch (err) {
...
async function getStatsAndPerformCopy(destStat, src, dest, opts) {
  const statFn = opts.dereference ? fs$l.stat : fs$l.lstat;
...
async function copyFile$1(srcStat, src, dest, opts) {
  await fs$l.copyFile(src, dest);
  if (opts.preserveTimestamps) {
    if (fileIsNotWritable$1(srcStat.mode)) {
      await makeFileWritable$1(dest, srcStat.mode);
    }
    const updatedSrcStat = await fs$l.stat(src);
    await utimesMillis(dest, updatedSrcStat.atime, updatedSrcStat.mtime);
  }
  return fs$l.chmod(dest, srcStat.mode);
}
function fileIsNotWritable$1(srcMode) {
  return (srcMode & 128) === 0;
}
function makeFileWritable$1(dest, srcMode) {
  return fs$l.chmod(dest, srcMode | 128);
}
async function onDir$1(srcStat, destStat, src, dest, opts) {
  if (!destStat) {
    await fs$l.mkdir(dest);
  }
  const promises = [];
  for await (const item of await fs$l.opendir(src)) {
    const srcItem = path$i.join(src, item.name);
    const destItem = path$i.join(dest, item.name);
    promises.push(
      runFilter(srcItem, destItem, opts).then((include) => {
        if (include) {
          return stat$4.checkPaths(srcItem, destItem, "copy", opts).then(({ destStat: destStat2 }) => {
  ...
  if (!destStat) {
    await fs$l.chmod(dest, srcStat.mode);
  }
}
async function onLink$1(destStat, src, dest, opts) {
  let resolvedSrc = await fs$l.readlink(src);
  if (opts.dereference) {
    resolvedSrc = path$i.resolve(process.cwd(), resolvedSrc);
  }
  if (!destStat) {
    return fs$l.symlink(resolvedSrc, dest);
  }
  let resolvedDest = null;
  try {
    resolvedDest = await fs$l.readlink(dest);
  } catch (e) {
    if (e.code === "EINVAL" || e.code === "UNKNOWN") return fs$l.symlink(resolvedSrc, dest);
    throw e;
  }
  if (opts.dereference) {
...
function copyDir(src, dest, opts) {
  const dir2 = fs$k.opendirSync(src);
  try {
    let dirent;
...
function remove$2(path2, callback) {
  fs$j.rm(path2, { recursive: true, force: true }, callback);
}
function removeSync$1(path2) {
  fs$j.rmSync(path2, { recursive: true, force: true });
}
var remove_1 = {
  remove: u$9(remove$2),
...
const emptyDir = u$8(async function emptyDir2(dir2) {
  let items;
  try {
    items = await fs$i.readdir(dir2);
  } catch {
    return mkdir$3.mkdirs(dir2);
  }
  return Promise.all(items.map((item) => remove$1.remove(path$g.join(dir2, item))));
});
function emptyDirSync(dir2) {
  let items;
  try {
    items = fs$i.readdirSync(dir2);
  } catch {
    return mkdir$3.mkdirsSync(dir2);
  }
  items.forEach((item) => {
    item = path$g.join(dir2, item);
...
async function createFile$1(file2) {
  let stats;
  try {
    stats = await fs$h.stat(file2);
  } catch {
  }
  if (stats && stats.isFile()) return;
  const dir2 = path$f.dirname(file2);
  let dirStats = null;
  try {
    dirStats = await fs$h.stat(dir2);
  } catch (err) {
    if (err.code === "ENOENT") {
      await mkdir$2.mkdirs(dir2);
      await fs$h.writeFile(file2, "");
      return;
    } else {
      throw err;
    }
  ...
  if (dirStats.isDirectory()) {
    ...
    await fs$h.readdir(dir2);
  }
}
function createFileSync$1(file2) {
  let stats;
  try {
    stats = fs$h.statSync(file2);
  } catch {
  }
  if (stats && stats.isFile()) return;
  const dir2 = path$f.dirname(file2);
  try {
    if (!fs$h.statSync(dir2).isDirectory()) {
      fs$h.readdirSync(dir2);
    }
  } catch (err) {
    if (err && err.code === "ENOENT") mkdir$2.mkdirsSync(dir2);
    else throw err;
  }
  fs$h.writeFileSync(file2, "");
}
var file$4 = {
...
async function createLink$1(srcpath, dstpath) {
  let dstStat;
  try {
    dstStat = await fs$g.lstat(dstpath);
  } catch {
  }
  let srcStat;
  try {
    srcStat = await fs$g.lstat(srcpath);
  } catch (err) {
    err.message = err.message.replace("lstat", "ensureLink");
    throw err;
  }
  if (dstStat && areIdentical$1(srcStat, dstStat)) return;
  const dir2 = path$e.dirname(dstpath);
  const dirExists = await pathExists$4(dir2);
  if (!dirExists) {
    await mkdir$1.mkdirs(dir2);
  }
  await fs$g.link(srcpath, dstpath);
}
function createLinkSync$1(srcpath, dstpath) {
  let dstStat;
  try {
    dstStat = fs$g.lstatSync(dstpath);
  } catch {
  }
  try {
    const srcStat = fs$g.lstatSync(srcpath);
    if (dstStat && areIdentical$1(srcStat, dstStat)) return;
  } catch (err) {
    err.message = err.message.replace("lstat", "ensureLink");
    throw err;
  ...
  const dir2 = path$e.dirname(dstpath);
  const dirExists = fs$g.existsSync(dir2);
  if (dirExists) return fs$g.linkSync(srcpath, dstpath);
  mkdir$1.mkdirsSync(dir2);
  return fs$g.linkSync(srcpath, dstpath);
}
var link = {
...
function symlinkPathsSync$1(srcpath, dstpath) {
  if (path$d.isAbsolute(srcpath)) {
    const exists3 = fs$f.existsSync(srcpath);
  ...
  const dstdir = path$d.dirname(dstpath);
  const relativeToDst = path$d.join(dstdir, srcpath);
  const exists2 = fs$f.existsSync(relativeToDst);
  if (exists2) {
    return {
      toCwd: relativeToDst,
      toDst: srcpath
  ...
  const srcExists = fs$f.existsSync(srcpath);
...
async function symlinkType$1(srcpath, type) {
  ...
  let stats;
  try {
    stats = await fs$e.lstat(srcpath);
  } catch {
    return "file";
  }
...
async function createSymlink$1(srcpath, dstpath, type) {
  let stats;
  try {
    stats = await fs$d.lstat(dstpath);
  } catch {
  }
  ...
  const dir2 = path$c.dirname(dstpath);
  if (!await pathExists$2(dir2)) {
    await mkdirs(dir2);
  }
  return fs$d.symlink(srcpath, dstpath, toType2);
}
function createSymlinkSync$1(srcpath, dstpath, type) {
  let stats;
  try {
    stats = fs$d.lstatSync(dstpath);
  } catch {
  }
  if (stats && stats.isSymbolicLink()) {
    const srcStat = fs$d.statSync(srcpath);
    const dstStat = fs$d.statSync(dstpath);
    if (areIdentical(srcStat, dstStat)) return;
  }
  ...
  const dir2 = path$c.dirname(dstpath);
  const exists2 = fs$d.existsSync(dir2);
  if (exists2) return fs$d.symlinkSync(srcpath, dstpath, type);
  mkdirsSync(dir2);
  return fs$d.symlinkSync(srcpath, dstpath, type);
}
var symlink = {
  createSymlink: u$3(createSymlink$1),
  createSymlinkSync: createSymlinkSync$1
};
const { createFile, createFileSync } = file$4;
...
async function outputFile$1(file2, data, encoding = "utf-8") {
  const dir2 = path$b.dirname(file2);
  if (!await pathExists$1(dir2)) {
    await mkdir.mkdirs(dir2);
  }
  return fs$c.writeFile(file2, data, encoding);
}
function outputFileSync$1(file2, ...args) {
  const dir2 = path$b.dirname(file2);
  if (!fs$c.existsSync(dir2)) {
    mkdir.mkdirsSync(dir2);
  }
  fs$c.writeFileSync(file2, ...args);
}
var outputFile_1 = {
...
function isParentRoot(dest) {
  const parent2 = path$9.dirname(dest);
  const parsedPath = path$9.parse(parent2);
...
const fs$9 = /* @__PURE__ */ getDefaultExportFromCjs(lib$2);
const defaultConfig = {
  theme: "light",
  language: "zh-CN",
  autoUpdate: true,
    ...
    if (config === true) {
      ...
      if (config.columns) {
        ...
        if (selectedColumns.length > 0) {
          ...
          }) : Object.keys(tableConfig.columns).filter((key2) => !selectedColumns.includes(key2));
        }
      } else {
...
function getDb() {
  ...
  const isDir = require$$1__default.extname(CONFIG_PATH2) === "";
  let dbPath;
  if (isDir) {
    const dataDir = require$$1__default.join(CONFIG_PATH2, "data");
    fs$9.ensureDirSync(dataDir);
    dbPath = require$$1__default.join(dataDir, "data.db");
  } else {
    fs$9.ensureDirSync(require$$1__default.dirname(CONFIG_PATH2));
    dbPath = CONFIG_PATH2;
  }
  const isNew = !fs$9.existsSync(dbPath);
  if (isNew) {
    console.log("📦 创建新的 SQLite 数据库:", dbPath);
    fs$9.writeFileSync(dbPath, "");
  } else {
...
function getProjectById(id) {
  const db2 = getDb();
  return db2.select().from(project).where(eq$3(project.id, id)).get();
}
function updateProject(id, data) {
  const db2 = getDb();
  return db2.update(project).set(data).where(eq$3(project.id, id)).returning();
}
function deleteProject(id) {
  const db2 = getDb();
  return db2.delete(project).where(eq$3(project.id, id)).returning();
}
function registerProjectHandlers() {
  ipcMain.handle("project:create", async (_2, projectData) => {
    try {
  ...
  ipcMain.handle("project:getById", async (_2, id) => {
    try {
      return await getProjectById(id);
    } catch (error2) {
      console.error("获取项目失败:", error2);
      throw error2;
    }
  });
  ipcMain.handle("project:update", async (_2, id, projectData) => {
    try {
  ...
  ipcMain.handle("project:delete", async (_2, id) => {
    try {
...
function getProjectBuild(projectId) {
  ...
  if (projectId) {
    return query.where(eq$3(projectBuild.projectId, projectId)).orderBy(desc(projectBuild.startTime)).all();
  }
  return query.orderBy(desc(projectBuild.startTime)).all();
}
function getProjectBuildById(id) {
  const db2 = getDb();
  return db2.select().from(projectBuild).where(eq$3(projectBuild.id, id)).get();
}
function updateProjectBuild(id, data) {
  const db2 = getDb();
  return db2.update(projectBuild).set(data).where(eq$3(projectBuild.id, id)).returning();
}
function deleteProjectBuild(id) {
  const db2 = getDb();
  return db2.delete(projectBuild).where(eq$3(projectBuild.id, id)).returning();
}
function registerProjectBuildHandlers() {
  ipcMain.handle("build:create", async (_2, projectBuildData) => {
    try {
      return await createProjectBuild(projectBuildData);
    } catch (error2) {
      console.error("创建项目日志失败:", error2);
      throw error2;
    }
  });
  ipcMain.handle("build:getAll", async (_2, projectId) => {
    try {
      return await getProjectBuild(projectId);
    } catch (error2) {
      console.error("获取项目日志列表失败:", error2);
      throw error2;
    }
  });
  ipcMain.handle("build:getById", async (_2, id) => {
    try {
      return await getProjectBuildById(id);
    } catch (error2) {
      console.error("获取项目日志失败:", error2);
      throw error2;
    }
  });
  ipcMain.handle("build:update", async (_2, id, projectBuildData) => {
    try {
      return await updateProjectBuild(id, projectBuildData);
    } catch (error2) {
      console.error("更新项目日志失败:", error2);
      throw error2;
    }
  });
  ipcMain.handle("build:delete", async (_2, id) => {
    try {
      return await deleteProjectBuild(id);
    } catch (error2) {
      console.error("删除项目日志失败:", error2);
      throw error2;
    }
  });
}
...
function getServerById(id) {
  return db.select().from(server).where(eq$3(server.id, id)).get();
}
function updateServer(id, data) {
  return db.update(server).set(data).where(eq$3(server.id, id)).returning();
}
function deleteServer(id) {
  return db.delete(server).where(eq$3(server.id, id)).returning();
}
...
function readdir(dir2, strict) {
  return new Promise((resolve2, reject2) => {
    fs$8.readdir(dir2, { withFileTypes: true }, (err, files) => {
      if (err) {
        switch (err.code) {
          case "ENOTDIR":
            if (strict) {
...
const require$$2$1 = /* @__PURE__ */ getAugmentedNamespace(async$1);
...
function arrayLikeKeys(value, inherited) {
  ...
  for (var key2 in value) {
    if (!(skipIndexes && (key2 == "length" || isIndex(key2, length)))) {
      result.push(key2);
    }
  }
  return result;
...
function assignInDefaults(objValue, srcValue, key2, object) {
  if (objValue === void 0 || eq$2(objValue, objectProto$4[key2]) && !hasOwnProperty$4.call(object, key2)) {
    return srcValue;
  }
  return objValue;
}
function assignValue(object, key2, value) {
  ...
  if (!(hasOwnProperty$4.call(object, key2) && eq$2(objValue, value)) || value === void 0 && !(key2 in object)) {
    object[key2] = value;
  }
}
...
function assocIndexOf$1(array, key2) {
...
function getMapData$1(map2, key2) {
...
Archiver$1.prototype._moduleSupports = function(key2) {
...
function ensureBuffer(input) {
  ...
  var hasNewBufferAPI = typeof Buffer$3.alloc === "function" && typeof Buffer$3.from === "function";
...
var StringDecoder = require$$4$1.StringDecoder;
...
class SSHManager {
  ...
  async executeCommand(connectionId, command) {
    ...
    return new Promise((resolve2, reject2) => {
      client.exec(command, (err, stream2) => {
        if (err) {
          reject2(new Error(`命令执行失败: ${err.message}`));
          return;
        }
        let output = "";
        ...
        stream2.on("error", (err2) => {
          reject2(new Error(`流错误: ${err2.message}`));
        });
      });
    });
  }
  ...
  async getSystemInfo(connectionId) {
    ...
    try {
      ...
      throw new Error(`获取系统信息失败: ${error2}`);
    }
  }
  /**
  ...
  async getCPUUsage(connectionId, interval = 1e3) {
    try {
      ...
      console.error("获取 CPU 使用率失败:", error2);
      return 0;
    }
  }
  /** 解析磁盘大小 */
  ...
  async createShell(connectionId) {
    ...
    return new Promise((resolve2, reject2) => {
      client.shell({ cols: 80, rows: 24 }, (err, stream2) => {
        if (err) {
          reject2(new Error(`创建shell失败: ${err.message}`));
          return;
        }
        resolve2({ stream: stream2, cols: 80, rows: 24 });
      });
    });
  }
  /** 断开连接 */
...
function registerSSHHandlers() {
  ipcMain.handle("ssh:connect", async (_2, config) => {
    try {
      return await sshManager.connect(config);
    } catch (error2) {
      console.error("SSH连接失败:", error2);
      throw error2;
    }
  });
  ipcMain.handle("ssh:execute", async (_2, connectionId, command) => {
    try {
      return await sshManager.executeCommand(connectionId, command);
    } catch (error2) {
      console.error("SSH命令执行失败:", error2);
      throw error2;
    }
  });
  ipcMain.handle("ssh:getSystemInfo", async (_2, connectionId) => {
    try {
      return await sshManager.getSystemInfo(connectionId);
    } catch (error2) {
      console.error("获取系统信息失败:", error2);
      throw error2;
    }
  });
  ipcMain.handle("ssh:createShell", async (event, connectionId) => {
    try {
      const { stream: stream2, cols, rows } = await sshManager.createShell(connectionId);
      stream2.on("data", (data) => {
      ...
      console.error("创建shell失败:", error2);
      throw error2;
    }
  });
  ipcMain.handle("ssh:disconnect", async (_2, connectionId) => {
    try {
      await sshManager.disconnect(connectionId);
      return true;
    } catch (error2) {
      console.error("断开SSH连接失败:", error2);
      throw error2;
    }
  });
  ipcMain.handle("ssh:isConnected", async (_2, connectionId) => {
  ...
  ipcMain.handle("ssh:test", async (_2, data) => {
    try {
...
function requireInternal() {
  ...
  var StringDecoder2 = require$$4$1.StringDecoder;
...
function requireUtf32() {
  ...
  Utf32Decoder.prototype.write = function(src) {
    ...
    if (overflow2.length > 0) {
      for (; i < src.length && overflow2.length < 4; i++)
        overflow2.push(src[i]);
...
const __dirname = path$l.dirname(fileURLToPath(import.meta.url));
process.env.APP_ROOT = path$l.join(__dirname, "..");
const VITE_DEV_SERVER_URL = process.env["VITE_DEV_SERVER_URL"];
const MAIN_DIST = path$l.join(process.env.APP_ROOT, "dist-electron");
const RENDERER_DIST = path$l.join(process.env.APP_ROOT, "dist");
process.env.VITE_PUBLIC = VITE_DEV_SERVER_URL ? path$l.join(process.env.APP_ROOT, "public") : RENDERER_DIST;
const iconPath = path$l.join(process.env.VITE_PUBLIC, "dot-forge.png");
let win;
function createWindow() {
  win = new BrowserWindow({
    ...
    // 最小高度限制
    webPreferences: {
      preload: path$l.join(__dirname, "preload.mjs")
      // 预加载脚本
    }
  });
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    win.loadFile(path$l.join(RENDERER_DIST, "index.html"));
  }
  win.on("maximize", () => {
